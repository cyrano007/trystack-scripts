...
function _prov_ca {
  docker-machine ssh ${OST_PRJ_NAME} sudo rm -rf /etc/ca_cfssl
  docker-machine ssh ${OST_PRJ_NAME} sudo mkdir /etc/ca_cfssl
  docker-machine scp ./config.json ${OST_PRJ_NAME}:/tmp/.
  docker-machine scp ./ca.json ${OST_PRJ_NAME}:/tmp/.
  docker-machine ssh ${OST_PRJ_NAME} sudo mv /tmp/*.json /etc/ca_cfssl
  docker-machine ssh ${OST_PRJ_NAME} sudo ls -rtl /etc/ca_cfssl
}

function _prov_intermediate {
...
function create_root_ca {
  ...
  docker run --rm -v /etc/ca_cfssl:/etc/cfssl cyrano007/cfssl genkey -initca ca.json | docker run --rm -i -v /etc/ca_cfssl:/etc/cfssl --entrypoint cfssljson cyrano007/cfssl -bare ca
}
...
function create_intermediate_ca {
  ...
  docker run --rm -v /etc/cfssl:/etc/cfssl cyrano007/cfssl sign -ca ca.pem -ca-key ca-key.pem -config config_intermediate.json intermediate_ca.csr | docker run --rm -i -v /etc/cfssl:/etc/cfssl --entrypoint cfssljson cyrano007/cfssl -bare intermediate
